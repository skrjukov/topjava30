...
public class JdbcMealRepository implements MealRepository {
    ...
    public Meal get(int id, int userId) {
        List<Meal> meals = jdbcTemplate.query("SELECT * FROM meal WHERE id=? and user_id=?",
    ...
    public List<Meal> getBetweenHalfOpen(LocalDateTime startDateTime, LocalDateTime endDateTime, int userId) {
        return jdbcTemplate.query( "SELECT * FROM meal WHERE user_id=?  AND datetime >=  ? AND " +
                 "datetime < ? ORDER BY datetime DESC",  ROW_MAPPER, userId, startDateTime, endDateTime);
    }
}
